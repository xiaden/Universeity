...
### T5 canonical selection and deduplication (Refiner reconciliation)

This artifact carries ONE canonical T5 pattern set. Per T7 R1, the canonical set is
**IP-1–IP-10** (source lines 1506–1818, the second T5 append). The first T5 set
(source lines 1056–1505, Patterns 1–10) is a supplementary detail source, not a
rival; it was NOT copied here to avoid duplicating a divergent contract. Uniquely
necessary content from the first T5 set is preserved as follows:

- **(a) `umd-evidence/` directory layout with fixed filenames** (first-T5 Pattern 8) —
  merged into T7 F8 (fixed layout: `docker-capability.txt`, `image-digests.txt`,
  `compose-up-batch-{1..4}-{attempt}.log`, `compose-reconcile-{n}.log`,
  `svc-inspect/{service}.json`, `compose-ps.txt`, `network-inspect.txt`,
  `sandbox-security.txt`, `escalation-verdict.txt`, `live-worker-gate.txt`,
  `public-boundary-gate.txt`, `public-boundary-after-restart-gate.txt`).
- **(b) Reconcile settle-window bound 60s/5s** (first-T5 Pattern 7) — merged into T7 F7;
  human-approved parameter (T4 Q3 / T8 Q3).
- **(c) Daemon-reachability bound 120s/5s** (first-T5 Pattern 2) — merged into T7 F2;
  human-approved parameter (T4 Q4 / T8 Q4).
- First-T5 Pattern 1 (job/step decomposition: `compose-start.sh` as sole compose owner
  inside a step sequence, preflight/diagnostics kept as separate steps) is subsumed by
  IP-1’s interpreter contract and T7 F1/F3.
- First-T5 Pattern 3’s intra-batch `COMPOSE_PARALLEL_LIMIT` floor-of-2 fallback is
  superseded by T6 §4 (stale on current Compose; `=1` valid) and T7 F3.
- All other first-T5 shapes (two-phase preflight, serialized batches, exact-marker
  retry + PIPESTATUS, one-shot state model, JWT order, post-up reconciliation,
  diagnostics schema, sandbox classification, hosted gate + escalation) are covered by
  IP-1…IP-10 and T7 F1–F10 with equivalent or stronger content; no unique hosted
  evidence, technology citation, or check date is lost (Task.md §40 item cross-references
  §40-3/4/21/31/33/35 from the first-T5 mapping table are subsumed by L1–L9 and the
  IP/T7 mapping tables).
...
**Artifact reconciliation record (Refiner).** This validated artifact was produced from
`artifacts/designs/process/ADVERSARIAL-universal-media-decomposer-plan-k-netns-workflow.md`
(source, 2832 lines) by copying the immutable prelude/ledger verbatim and the substantive
turn sections in strict canonical turn order. No design content, evidence, or citation was
altered; the only structural changes are (1) canonical T5 selection (IP-1–IP-10), (2) T6
placed before T7 to restore strict T1→T8 order, and (3) this manifest plus the T5
deduplication note. The source file's physical order anomaly (duplicate T5 + T7-before-T6)
is recorded in the source as T8 U8 and T7 R1 and is resolved here. T7's "Section-state
transparency" note (written when no T6 existed in the file) and T8's U8 are preserved
verbatim and must be read in that historical context.

### Source ranges (source file line numbers)

| Section | Source range | Status in this artifact |
|---|---|---|
| Immutable prelude/ledger | 1–203 | Copied verbatim (dst 1–203) |
| T1 Ideator — Proposed Approaches | 204–282 | Copied verbatim (dst 204–282) |
| T2 Counter-Ideator — Critique | 283–452 | Copied verbatim (dst 283–452) |
| T3 Ideator — Refined Approaches | 453–768 | Copied verbatim (dst 453–768) |
| T4 Counter-Ideator — Surviving Concerns | 769–1055 | Copied verbatim (dst 769–1055) |
| T5 Improver — Implementation Patterns, first set (Patterns 1–10) | 1056–1505 | Duplicate, non-canonical — not copied; uniquely necessary details merged into T7 F2/F7/F8 and recorded in the T5 deduplication note |
| T5 Improver — Implementation Patterns, second set (IP-1–IP-10) | 1506–1818 | Canonical T5 — copied verbatim (dst 1056–1368) |
| T6 Counter-Improver — Pattern Risk Assessment | 2236–2512 | Copied verbatim; placed before T7 (dst 1400–1676) |
| T7 Improver — Final Patterns and Mitigations | 1819–2235 | Copied verbatim; placed after T6 (dst 1677–2094) |
| T8 Counter-Improver — Open Risks and Human Questions | 2513–2832 | Copied verbatim (dst 2095–2415) |

### Canonical heading order (this artifact)

1. `## T1 Ideator — Proposed Approaches`
2. `## T2 Counter-Ideator — Critique`
3. `## T3 Ideator — Refined Approaches`
4. `## T4 Counter-Ideator — Surviving Concerns`
5. `## T5 Improver — Implementation Patterns` (canonical IP-1–IP-10) + `### T5 canonical selection and deduplication (Refiner reconciliation)`
6. `## T6 Counter-Improver — Pattern Risk Assessment`
7. `## T7 Improver — Final Patterns and Mitigations`
8. `## T8 Counter-Improver — Open Risks and Human Questions`
9. `## Validation Manifest`

### R18 (`33228898244`) status/tier caveat

- The shared ledger's hosted-evidence row still reads "NOT established — do not invent it."
- T4/T5/T6/T7/T8 upgrade this to: **corroborated at the coordination/analysis tier** by two
  independent corpus entries — exec-manager L105 (02:29:29Z) and researcher L8 (02:30:38Z):
  full split topology boots, `/v1/ready` PASS, worker registration PASS, live Hatchet suite
  6 pass / 4 FAIL (toolchain/test-isolation, NOT netns), netns marker appeared once and was
  swallowed by `| tee` (pipefail-OFF inference).
- **Tier-1 GitHub re-retrieval is pending and binding** (Phase 6 / Exec-Manager obligation;
  T7 AT-10, T8 U7/Q2): the ledger row must be updated from the GitHub run before the
  amendment is finalized. The "pipefail was OFF" reading is an inference — the R15
  compose-exit-0 class produces the identical observable; T7 F1's explicit status contract
  makes the design independent of the inference, but tier-1 retrieval should settle it.

### Unresolved risks (final register, T8 U1–U8)

- **U1 (BLOCKING):** PIPESTATUS capture unreachable under `set -e` as written in T7 F1 —
  must be rewritten with the shielding idiom `rc=0; docker compose … 2>&1 | tee "$log" ||
  rc=${PIPESTATUS[0]}` (or `if`/`!` shielding) before DDAuthor writes the amendment (T8
  U1, AT-1 amended).
- **U2 (HIGH):** `up`-level hang / one-shot re-trigger (`service_completed_successfully`;
  docker/compose#10985/#11808/#12134/#8913/#9273) — encode `--no-deps` on batches 3/4 and
  an explicit `timeout` on every compose `up` into F3/F5 (T8 Q7, AT-12).
- **U3 (MEDIUM):** job-level `defaults: run: shell: bash` blast radius — per-step pipefail
  audit required (T8 Q8, AT-11).
- **U4 (MEDIUM/HIGH):** seccomp profile honesty — actual `sandbox-seccomp.json` is an
  ungated ~415-syscall allowlist, not "moby default + pivot_root"; human security sign-off
  and AppArmor/userns diagnostics required (T8 Q6, AT-15).
- **U5 (MEDIUM):** upload `if-no-files-found: warn` → `error`; verdict file must be written
  on all death paths (AT-14).
- **U6 (MEDIUM):** Docker version marker drift — runner image 28.0.4 today, v29 re-update
  pending (actions/runner-images#14105); re-validate the F4 regex on major-version change
  (AT-13).
- **U7 (OBLIGATION):** R18 tier-1 re-retrieval blocks amendment finalization (AT-10).
- **U8 (PROCESS):** source artifact order anomaly — resolved in this artifact; consumers
  must read T6 before T7.

### Human questions (T8 Q1–Q8)

Q1 sandbox-runner role/env (blocks next hosted run; recommended option a — mandatory gate
with full Hatchet env parity); Q2 R18 tier-1 retrieval (blocks finalization); Q3 reconcile
settle-window (proposed 60s/5s); Q4 daemon-reachability budget (proposed 120s/5s); Q5
`COMPOSE_PARALLEL_LIMIT=1` via env; Q6 seccomp profile sign-off; Q7 `--no-deps` + `timeout`
on every `up`; Q8 job-level shell default audit.

### Verdict

**Workflow-only Plan K amendment; NO architecture/topology change** (T1–T8 consensus; T8
"Final decision"). The amendment carries T7's canonical F1–F10 + F-R18 with the U1–U8
resolutions and Q1–Q8 human decisions. L1–L9 preserved; the forbidden set (Hatchet Lite,
DinD/socket mounts, privileged/unconfined broad bypass, skipping sandbox,
optional/trigger-level gate, fake readiness, second scheduler, host networking, blanket
retry, `--wait` sole gate) is never re-introduced. Topology re-opens only on new hosted
evidence (tier-1 R18 contradiction, or the exact anchored marker recurring on a clean
runner under the corrected F1 status contract with the 3-attempt budget exhausted).

### Mandatory-item check

- [x] Immutable prelude/ledger preserved verbatim (dst 1–203)
- [x] T1–T8 all present in strict order
- [x] Canonical T5 = IP-1–IP-10; first-T5 unique details preserved via T7 F2/F7/F8 + dedup note
- [x] All T6/T7/T8 risk corrections included (U1–U8, F1–F10 + F-R18, AT-1..AT-15)
- [x] Exact hosted evidence and citations preserved with check dates
- [x] R18 status/tier caveat recorded
- [x] Unresolved risks surfaced with human context
- [x] Verdict recorded (workflow-only, no topology change)
