...
- **Text/book/sequential art:** TXT, Markdown, EPUB, and text-viable PDF produce deterministic document/chapter/section/paragraph/sentence/token-span segments; dialogue/narration, entity and alias candidates, speaker candidates, events, locations, relationships, and semantic segments retain evidence links. Image-only PDF pages route to the raster/OCR path. Page/panel/region/bubble/caption structure is emitted where the source supports it.
